...
class DocumentGenerator:
    ...
    def _add_section_heading(self, doc: Document, title: str, emoji: str = ""):
    ...
    def _add_description_box(self, doc: Document, text: str):
    ...
    def _add_numbered_list(self, doc: Document, items: List[str], color: RGBColor = None):
    ...
    def generate(self, job_description: str, pain_points: List[str],
        ...
        Thread-safe: all state is local — no shared mutable instance state.
        """
        doc = Document()
        ...
        self._add_subtitle(doc, "Work Process Analysis")

        # Job Description section
        self._add_section_heading(doc, "Job Description", "🎯")
        self._add_description_box(doc, job_description)

        # Pain Points section
        self._add_section_heading(doc, f"Pain Points ({len(pain_points)} identified)", "😓")
        paragraph = doc.add_paragraph()
        ...
        paragraph.space_after = Pt(8)
        self._add_numbered_list(doc, pain_points, self.WARNING_COLOR)

        # Gain Points section
        self._add_section_heading(doc, f"Gain Points ({len(gain_points)} identified)", "🌟")
        paragraph = doc.add_paragraph()
        ...
        run.font.color.rgb = RGBColor(107, 114, 128)
        paragraph.space_after = Pt(8)
        self._add_numbered_list(doc, gain_points, self.SUCCESS_COLOR)
